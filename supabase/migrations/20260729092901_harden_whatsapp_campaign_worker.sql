alter table public.whatsapp_campaigns
  add column next_batch_at timestamptz not null default now();

create index whatsapp_campaigns_worker_claim_idx
on public.whatsapp_campaigns (status, next_batch_at, scheduled_at, created_at)
...
begin
  ...
  if p_max_batch_duration_seconds < 1
    or p_max_batch_duration_seconds > 300 then
    raise exception 'Worker batch duration must be between 1 and 300 seconds'
      using errcode = '22023';
  end if;
  ...
  order by
    campaigns.scheduled_at nulls first,
    campaigns.started_at nulls first,
  ...
  -- Settle contacts that are no longer sendable. When a test allowlist is
  -- supplied, non-test contacts are skipped rather than repeatedly claimed.
  update public.whatsapp_campaign_recipients as recipients
  set
    status = 'skipped',
    processing_started_at = null,
    failure_reason = case
      when contacts.consent_status <> 'confirmed'
        or contacts.opted_out_at is not null
  ...
  where recipients.campaign_id = target_campaign.id
    ...
    and (
      contacts.consent_status <> 'confirmed'
      or contacts.opted_out_at is not null
      or (
  ...
  claim_limit := least(
    ...
    greatest(
      1,
      (p_max_batch_duration_seconds / greatest(target_campaign.delay_seconds, 1)) + 1
    )
  );
  ...
  with candidates as (
    ...
    where recipients.campaign_id = target_campaign.id
      ...
      and (
        p_allowed_phone_numbers is null
        or contacts.phone_number = any(p_allowed_phone_numbers)
      )
  ...
  set next_batch_at = now() +
    make_interval(
      secs => greatest(target_campaign.delay_seconds, 1) *
        greatest(claimed_count, 1)
    )
  where id = target_campaign.id;
