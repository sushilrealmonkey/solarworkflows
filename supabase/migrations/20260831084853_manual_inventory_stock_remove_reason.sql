-- Direct stock-in does not require a reason. Optional notes remain available
-- for teams that want to record context without blocking a quick stock add.

drop function if exists public.add_inventory_stock(
  uuid,
  numeric,
  date,
  text,
...
revoke execute on function public.add_inventory_stock(
  uuid,
  numeric,
  date,
  uuid,
