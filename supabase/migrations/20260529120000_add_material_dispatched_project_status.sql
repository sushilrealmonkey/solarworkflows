...
begin
  ...
  if new_status = 'material_dispatched' then
    if project_record.quotation_id is null then
      raise exception 'A linked quotation is required to dispatch BOM material'
        using errcode = '23514';
    end if;
    ...
    if not exists (
      ...
      if not found then
        raise exception 'Linked quotation was not found for this project'
          using errcode = 'P0002';
      end if;
      ...
      if inserted_count = 0 then
        raise exception 'No inventory-linked BOM items found to dispatch'
          using errcode = '23514';
      end if;
    end if;
  end if;
