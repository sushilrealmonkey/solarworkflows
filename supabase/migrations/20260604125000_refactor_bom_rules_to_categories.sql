-- Refactor BOM Rules back to category-level template requirements.
-- Actual products, brands, and models are assigned later during quotation BOM generation.

alter table public.bom_template_lines
drop constraint if exists bom_template_lines_product_required_check;

alter table public.bom_template_lines
drop constraint if exists bom_template_lines_product_id_fkey;

drop index if exists public.bom_template_lines_product_id_idx;
