...
begin
  ...
  if sale_record.status = 'cancelled' then
    raise exception 'Cancelled B2B sales cannot be dispatched'
      using errcode = '23514';
  end if;
...
revoke execute on function public.confirm_b2b_sale(uuid) from public, authenticated;
revoke execute on function public.cancel_b2b_sale(uuid) from public, authenticated;
