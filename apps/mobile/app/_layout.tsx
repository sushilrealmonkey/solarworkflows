...
export default function RootLayout() {
  ...
  useEffect(() => { if (session === undefined) return; const inAuth = segments[0] === "(auth)"; const inEnrollment = segments.join("/").includes("enrollment"); if (!session && !inAuth) router.replace("/(auth)/login"); if (session && inAuth && !inEnrollment) void mobileApi.session().then(() => router.replace("/(tabs)/today")).catch((error) => { if (error instanceof ApiError && error.code === "ACCOUNT_UNASSIGNED") router.replace("/(auth)/enrollment"); }); }, [session, segments, router]);
