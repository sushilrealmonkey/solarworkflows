...
export function ProjectFieldAssignments({ projectId }: { projectId: string }) {
  ...
  const assignedStaffNames = options
    .filter((option) => option.is_assigned)
    .map((option) => option.full_name ?? "Field Staff");

  if (!loading && assignedStaffNames.length > 0) {
    return (
      <p className="text-sm font-semibold text-slate-900" title="Assigned field staff">
        Field Staff - {assignedStaffNames.join(", ")}
      </p>
    );
  }
  ...
  return (
    <section className="rounded-lg border border-stone-200 bg-white p-3 shadow-sm sm:p-4">
      <div>
        <h2 className="text-sm font-semibold text-slate-950">Field Staff assignment</h2>
        <p className="mt-0.5 text-xs text-slate-600">
          Assigned staff see this project after it reaches Installation Scheduled.
        </p>
      </div>
      {loading ? <div className="mt-3"><LoadingSkeleton /></div> : null}
      {!loading && options.length === 0 ? (
        <div className="mt-3">
          <EmptyState
            title="No active Field Staff"
            description="Create or activate a staff user with the Field Staff role first."
          />
        </div>
      ) : null}
      {!loading && options.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {options.map((option) => (
            <label
              key={option.id}
              className="flex items-center gap-2 rounded-lg border border-stone-200 p-2.5 text-sm"
            >
              <input
                type="checkbox"
              ...
              <span>
                <span className="block font-semibold text-slate-900">
                  {option.full_name ?? "Field Staff"}
                </span>
                <span className="text-xs text-slate-500">{option.phone ?? "No phone"}</span>
              </span>
            </label>
          ))}
        </div>
