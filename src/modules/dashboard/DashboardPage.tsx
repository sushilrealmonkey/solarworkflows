                ...
                {dueItems.length === 0 ? (
                  ...
                  <div className="mt-3 space-y-2">
                    {dueItems.slice(0, 4).map((item) => {
                      ...
                      const projectMilestone = !isB2BSale
                        ? paymentDueProjectMilestone(item)
                        : null;
                      return (
                        <Link
                ...
                {overduePayments.slice(0, 3).map((payment) => (
                  ...
                  >
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-600">
                      {payment.source_code ?? payment.source_name ?? "Record"}
                      {paymentDueProjectMilestone(payment)
                        ? ` · ${paymentDueProjectMilestone(payment)}`
                        : ""}
                      {` · Due ${formatDate(payment.payment_due_on)} · ${payment.days_overdue ?? 0} days overdue`}
                    </span>
...
function paymentDueItemKey(item: PaymentDueItem) {
  return [
    item.source_type,
    item.source_id,
    item.payment_due_on,
...
function paymentDueProjectMilestone(item: PaymentDueItem) {
  if (item.source_type !== "project" || !item.source_name?.includes(" · ")) {
    return null;
  }

  const labelParts = item.source_name.split(" · ");
  return labelParts[labelParts.length - 1] || null;
}
