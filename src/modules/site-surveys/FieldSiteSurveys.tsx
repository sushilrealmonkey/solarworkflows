...
export function FieldSiteSurveyDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    {survey && form ? <>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm"><h2 className="font-semibold text-slate-950">Next Step</h2><div className="mt-4 flex flex-wrap gap-2"><SiteSurveyMapLinkButton survey={survey} />{nextStatus ? <Button disabled={saving} onClick={updateStatus}>Update Status</Button> : <p className="text-sm text-slate-600">No status update is available. Completed work is read-only.</p>}</div></section>
    </> : null}
  </div>;
}
